import type PokerCard from "../../domain/card/PokerCard";
import type Table from "../../domain/table/Table";
import { Resolutions } from "../../resolutions";
import { type ICard } from "../../domain/card/Card";
import type Player from "./domain/player/Player";
import { SecretHand } from "./domain/hand";
...
export const tableRules = {
  canPlayCards: (table: Table, cards: PokerCard[]) => {
    ...
    return (
      !topStackCard ||
      cardRules.xCanBePlayedAfterY({ x: firstPlayedCard, y: topStackCard })
    );
  },
  canDiscard: (table: Table, cards: PokerCard[]) => {
    ...
    for (let i = 0; i < 4 - nPlayedCards && equalValue; i++) {
      ...
      equalValue =
        currentStackCard && currentStackCard.number === playedCardNumber;
    }
  ...
  canPlaySomething: (table: Table, player: Player) => {
    ...
    const canPlanAnyCardFromHand = hand.cards.some(
      (c: ICard | null) => cardRules.xCanBePlayedAfterY({ x: c!, y: topCard }));

    return canPlanAnyCardFromHand;
  }
};
